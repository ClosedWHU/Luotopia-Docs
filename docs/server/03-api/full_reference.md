# Luotopia Server 完整 API 接口参考手册

本文档包含 Luotopia Server 所有已注册接口的详细定义，供客户端开发及第三方集成参考。
...
- **认证**: Bearer Token
- **请求体 (Body)**:
  - `platform` (string): `android`, `ios`, 或 `web`。
  - `push_token` (string): 推送服务提供的 Token。
- **返回**: 204 No Content。
...
- **详情**: `GET /api/v1/materials/{uid}`

### 4.2 上传
- **端点**: `POST /api/v1/materials/upload`
- **认证**: Bearer Token
- **请求体**: `multipart/form-data` 包含 `file`, `title`, `course_uid`, `description`。
