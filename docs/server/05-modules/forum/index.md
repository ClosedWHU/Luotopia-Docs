...
1.  **[治理与规则 (Governance)](governance.md)**: 用户角色、管理员白名单、禁言逻辑及审核体系。
2.  **[内容系统 (Content & Search)](content.md)**: 帖子/回复的存储结构、标签系统及基于热度的搜索算法。
3.  **[互动流 (Interactions)](interaction.md)**: 点赞、踩、收藏及 Redis 缓存同步机制。
4.  **[内容安全 (Moderation)](moderation.md)**: 举报流、申诉处理及启发式 AI 自动审核。
5.  **[运营工具 (Operations)](operations.md)**: 通知系统、邀请码配额及校友验证流程。
