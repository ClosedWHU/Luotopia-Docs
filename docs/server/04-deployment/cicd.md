...
# 使用官方 Go 构建镜像
docker build \
  -t ghcr.io/ClosedWHU/luotopia-server:latest \
  -t ghcr.io/ClosedWHU/luotopia-server:v$(git describe --tags) \
  -f Dockerfile \
  .
```
...
**推送到 GHCR**：
```bash
docker push ghcr.io/ClosedWHU/luotopia-server:latest
docker push ghcr.io/ClosedWHU/luotopia-server:v$(git describe --tags)
```
...
services:
  luotopia-server:
    image: ghcr.io/ClosedWHU/luotopia-server:latest
    restart: always
    ports:
      - "8080:8080"
    environment:
...
# 自动回滚到上一个标签版本
docker service update --image ghcr.io/ClosedWHU/luotopia-server:v1.2.3 luotopia-server
```
...
# 2. 切换到特定版本
docker pull ghcr.io/ClosedWHU/luotopia-server:v1.2.2
docker service update --image ghcr.io/ClosedWHU/luotopia-server:v1.2.2 luotopia-server
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Build and push
        run: |
          docker build -t ghcr.io/ClosedWHU/luotopia-server:latest .
          docker push ghcr.io/ClosedWHU/luotopia-server:latest
