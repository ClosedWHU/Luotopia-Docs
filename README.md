...
| **`/ebike`** | 电单车接口参考（公开脱敏版） | `ebike-docs/` |
...
| Go / 部署 | `server-docs/`（`architecture` / `development` / `api` / `deployment` / `modules` / `meta`） |
| 电单车接口参考 | `ebike-docs/`（公开脱敏版；完整未脱敏分析在私有仓库 `whu-ebike-re`，改动须两侧同步脱敏） |

侧栏顺序靠各页 `sidebar_position` + 目录 `_category_.json` 的 `position`（四套均为 autogenerated）。
...
**服务端模块**：分类 身份(1) 论坛(2) 课程(3) 食堂(4) 搜索(5) 基础设施(15) 内部服务(16)；散页 管理(6) → 课表/日历/教室/校园边界(7-10) → 资料/通知(11-12) → 系统/外部面(13-14) → 天气(17) 即时通讯(18)
...
- 服务端：[已移除与迁移](./server-docs/meta/removed_and_migrated.md)
...
| 业务服 vs 官网 URL | `settings.md` | `auth.md`、`api-integration.md` | `campus_proxies` / `system` |
